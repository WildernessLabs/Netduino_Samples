...
namespace DehydratorRemote
{
    public class MapleClient
    {
        ...
        protected async Task<bool> TogglePowerAsync(string command, string hostAddress)
        {
        ...
        protected async Task<bool> GetStatusAsync(string command, string hostAddress)
        {
            var client = new HttpClient();
            client.BaseAddress = new Uri("http://" + hostAddress + "/");
            client.Timeout = TimeSpan.FromSeconds(5);
